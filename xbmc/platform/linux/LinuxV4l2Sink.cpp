...
bool CLinuxV4l2Sink::Init(int buffersCount = 0) {
  ...
  if (buffersCount <= 0 && m_Type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
    ...
    buffersCount = (int)(ctrl.value * 1.5) - buffersCount; //Most of the time we need 50% more extra capture buffers than device reported would be enough
  }
...
bool CLinuxV4l2Sink::QueryBuffers() {
  ...
  for(int i = 0; i < m_NumBuffers * m_NumPlanes; i++)
    m_Addresses[i] = (unsigned long)MAP_FAILED;
...
bool CLinuxV4l2Sink::MmapBuffers() {
  for(int i = 0; i < m_NumBuffers * m_NumPlanes; i++) {
    if(m_Planes[i].length) {
      m_Addresses[i] = (unsigned long)mmap(NULL, m_Planes[i].length, V4L2_TYPE_IS_OUTPUT(m_Type) ? PROT_WRITE : PROT_READ, MAP_SHARED, m_Device->device, m_Planes[i].m.mem_offset);
      if (m_Addresses[i] == (unsigned long)MAP_FAILED) {
        CLog::Log(LOGERROR, "%s::%s - Cannot MMap plane %d at offset %d with length %d. Device %s, Type %d, Memory %d", CLASSNAME, __func__, i, m_Planes[i].m.mem_offset, m_Planes[i].length, m_Device->name, m_Type, m_Memory);      
        return false;
      }
...
bool CLinuxV4l2Sink::StreamOn(int state) {
  if(ioctl(m_Device->device, state, &m_Type)) {
    if (state!=VIDIOC_STREAMOFF)
      CLog::Log(LOGERROR, "%s::%s - Error setting device state %x, Device %s, Type %d.", CLASSNAME, __func__, state, m_Device->name, m_Type);
    return false;
  }
